  ...
  public class StableTrieBase(args : Args) {
    ...
    public let storePointer : (region : Region.Region, offset : Nat64, child : Nat64) -> () = switch (pointer_size_) {
      ...
      case (6) func(region, offset, child) {
        ...
        Region.storeNat16(region, offset +% 4, natWrap16(nat64toNat(child >> 32)));
      };
      case (5) func(region, offset, child) {
        Region.storeNat32(region, offset, nat64to32(child & 0xffff_ffff));
        Region.storeNat8(region, offset +% 4, natWrap8(nat64toNat(child >> 32)));
      };
      case (4) func(region, offset, child) = Region.storeNat32(region, offset, nat64to32(child));
      case (2) func(region, offset, child) = Region.storeNat16(region, offset, natWrap16(nat64toNat(child)));
    ...
    public func newLeaf(region : Region, key : Blob) : ?Nat64 {
      ...
      Region.storeBlob(region.region, getLeafOffset(leaf), key);
      ?((leaf << 1) | 1);
    };
