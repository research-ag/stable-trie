...
| metric                                    | source                               |
| ----------------------------------------- | ------------------------------------ |
| `stable_trie_pointer_size`                | constructor `pointer_size`           |
| `stable_trie_value_size`                  | constructor `value_size`             |
| `stable_trie_key_size`                    | constructor `key_size`               |
| `stable_trie_aridity`                     | constructor `aridity`                |
| `stable_trie_root_aridity`                | constructor `root_aridity`, resolved |
...
The used/total split is exposed via a `kind` label on `stable_trie_node_count` and `stable_trie_leaf_count`, so Prometheus queries can `sum by (kind)` or filter with `{kind="used"}` directly. The two region page counts share the `stable_trie_region_pages` family, split by a `type` label. `stable_trie_root_aridity` reports the resolved value — equal to `stable_trie_aridity` when `null` was passed to the constructor.
